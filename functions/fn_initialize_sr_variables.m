function [sr] = fn_initialize_sr_variables(input, settings, sim)
...
%% Step 1: Pre-allocate memory for storage containers 
    ...
    sr.imports_used                    = zeros(settings.Nstep+1, input.sectors.N);
...
%         sr.total_ouptut                          = input.econ.total_output_pre_eq *  settings.exchange_rate;
        ...
        sr.assets                                = input.econ.FA_wHousing_pre_eq  .* settings.exchange_rate;
        sr.price                                 = ones(settings.Nstep+1,  input.sectors.N);
        sr.imports(1, :)                         = input.econ.imports_pre_eq       *  settings.exchange_rate .* input.import_loss.import_fraction_curve(:,1)';
...
%         sr.exports(1, :)                         = input.econ.exports_pre_eq * settings.exchange_rate - input.econ.imports_pre_eq' * settings.exchange_rate;
        ...
        sr.IO_norm                               = input.IO./repmat(sr.production(1, :) - input.econ.imports_pre_eq, input.sectors.N, 1);
        sr.demand(1, :)                          = sr.production(1, :);
        
        sr.dom_production(1, :)                  = sr.production(1, :) - input.econ.imports_pre_eq;
        
        sr.production_cap(1, :)                  = sr.production(1, :);
        sr.value_added(1, :)                     = sr.production(1, :) - sr.inter_purchases(1, :) - input.econ.imports_pre_eq;  

        % Orders, stocks
            for i = 1:input.sectors.N  
                for j = 1:input.sectors.N                          
                    sr.stock(i,j,1)   = input.IO(i,j) * input.params.n_stock(i, sim)*settings.dt;
        ...
        % Household budget modeling & macroeconomic effect
            sr.profit(1, :)                           = sr.production(1, :) - (sr.inter_purchases(1, :) + (settings.wage * sr.labor_comp(1, :)) + input.econ.imports_pre_eq);
            sr.macro_effect(1)                        = 1;
        ...
        % Losses and reconstruction demand
            sr.direct_losses_matrix                   = input.loss.loss_per_sector .* settings.ampl;
...
disp("TEMP comment : The sr struct contains " + num2str(length(fieldnames(sr))) + " variables.")
